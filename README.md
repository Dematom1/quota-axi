...
- For each credential probed, it sends one redirect-disabled `GET` to `https://api.fireworks.ai/v1/accounts/<account>/quotas` with the key as a `Bearer` token and a 15 second deadline, plus at most one `GET https://api.fireworks.ai/v1/accounts` when the account id had to be discovered. A non-empty quota-list `nextPageToken` fails the read with `fireworks_quota_incomplete`; pages are not followed or published as a partial reading. A non-object quota response fails with `schema_invalid`. It never writes credentials or changes vendor state, never calls the quota update endpoint, and never touches billing, deployment, or API-key management operations.
