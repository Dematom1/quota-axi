...
| Fireworks AI           | Reads the account's quota list from the vendor's first-party control-plane endpoint and reports one `quota:<id>` window per quota (for example `quota:h100-us-iowa-1`), whose `percentUsed` is that quota's own `usage` over its own enforced `value`. The endpoint reports no cycle and no reset, so every window is `unknown` kind and pace, runway, and selection stay unknown; a quota with no usable ratio is named in `state.untrustedWindowIds` instead of becoming a window. An account that declares no quotas is a fresh report with an empty `windows` list rather than invented percentages.                                                                                                                                                                                                                                                                                                                                                                          |
...
- The account id is the path segment of the quota URL, so it must be resolved before any quota request. quota-axi takes it from the same store the key came from - `FIREWORKS_ACCOUNT_ID` for the environment key, `account_id` for the `auth.ini` login - and never pairs one store's key with another store's account. When a store supplies a key but no account id, it resolves the id from the vendor's own read-only account listing and uses it only when exactly one account is returned; an empty or ambiguous listing is reported as `fireworks_account_unresolved` rather than guessed, because reading the wrong account would report another account's numbers as this one's. Set `FIREWORKS_ACCOUNT_ID` to name the account explicitly. An account id that is not a bare path segment is rejected before it can reach a URL.
- `auth.ini` also holds SSO material (`id_token`, `refresh_token`, `client_id`, `cognito_domain`, `issuer_url`). quota-axi's reader is a narrow whitelist walker that keeps only `api_key` and `account_id`, so nothing else in that file is ever held in memory, and it reads both the vendor's bare `key = value` layout and a section-headed one. Environment, template, and command references are not resolved or executed, so a key that holds one is reported as an `env` or `fireworks:auth.ini` source problem rather than sent as a header value. Key values are never logged, rendered, or cached. `FIREWORKS_AUTH_INI` overrides the login file path.
- It sends one redirect-disabled `GET` per read to `https://api.fireworks.ai/v1/accounts/<account>/quotas` with the key as a `Bearer` token and a 15 second deadline, plus at most one `GET https://api.fireworks.ai/v1/accounts` when the account id had to be discovered. It writes nothing, never calls the quota update endpoint, and never touches billing, deployment, or API-key management operations.
- What the numbers mean and do not mean: each row is one Fireworks quota's current `usage` as a percentage of the `value` currently enforced on the account, which the vendor may set below the approved `maxValue`. They are independent resource caps with no cycle - GPU quota per accelerator type and placement, the account-wide request-rate ceiling, and the monthly spend limit - so quota-axi reports each one on its own and claims no combined effective remaining, pace, runway, or selection for Fireworks. A GPU quota's percentage is concurrent capacity in use, not spend, and none of these rows is a token or credit balance.
- HTTP 401 is a definitive rejection of that key and falls through to the next store; when every readable key is rejected the report is `auth_required`. HTTP 403 is treated differently: the key was accepted and the quota operation refused, so quota-axi reports `state.status: unavailable` with `authStatus: usable` and no windows, never a sign-out - a console key without quota-read permission is not a logged-out account. Transport, timeout, rate-limit, redirect, decoding, and server failures stop without credential switching and never become authentication verdicts.
- Fireworks API keys do not expire and carry no refresh token, so there is nothing to renew and Fireworks has no delegated refresh. quota-axi also keeps no stale-cache fallback for Fireworks: the account a key names is not derivable from a cached snapshot, so a superseded reading is never served for an account it may not describe.
